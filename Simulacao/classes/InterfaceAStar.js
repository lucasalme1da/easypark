...
export default class InterfaceAStar {
    constructor({
        ...
        if (
            typeof nomeAtributoPosicao != "string" ||
            typeof nomeAtributoConexoes != "string"
        )
            throw new Error(
                "nomeAtributoPosicao e nomeAtributoConexoes devem ser strings"
            )
    ...
    removerNo() {
        this.raycaster.setFromCamera(this.mouse, this.camera)
        const data = this.raycaster.intersectObjects(
            this.nos.map((no) => no.mesh)
        )
        if (data.length == 0) {
    ...
    conectarNos() {
        ...
        if (this.nosParaConectar.length == 0) {
            ...
            console.log(
                "Segundo nó ",
                objeto.dono,
                ". Precione C para conectar os nós selecionados ou clique em qualquer lugar para cancelar."
            )
        } else {
            this.nosParaConectar.forEach((no) =>
                no.mesh.material.color.setHex(0x33ff99)
            )
            this.nosParaConectar = []
    ...
    conectarNosSelecionados() {
        ...
        this.nosParaConectar = []
        this.desenharLinha(
            primeiroNo[this.nomeAtributoPosicao],
            segundoNo[this.nomeAtributoPosicao],
            this.cena
        )
    ...
    iniciarLinhas() {
        this.nos.forEach((no) => {
            ...
            no[this.nomeAtributoConexoes]
                ...
                .forEach((noConectado) => {
                    linha = this.desenharLinha(
                        no[this.nomeAtributoPosicao].clone(),
                        noConectado[this.nomeAtributoPosicao].clone(),
                        this.cena
                    )
                })
        })
