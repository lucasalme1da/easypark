...
const { CubicBezierCurve3, Vector3, Raycaster, LineBasicMaterial, Line, BufferGeometry, SphereGeometry, MeshBasicMaterial, Mesh, Vector2 } = require("three")
export default class InterfaceAStar {
  constructor({ nomeAtributoPosicao, nomeAtributoConexoes, funcaoConectarNos, funcaoRemoveNos, funcaoAdicionaNos, nos, camera, cena, base, canvas }) {
    ...
    this.input.KeyU = () => this.desenharCurva()
  ...
  criarMesh(no) {
    const geometriaEsfera = new SphereGeometry(0.2, 12, 12)
  ...
  pegarPontosDeCurva(pontos, numeroPontos = 50) {
    const [a0, a1, a2] = pontos
    const curve = new CubicBezierCurve3(a0.posicao.clone(), a1.posicao.clone(), a1.posicao.clone(), a2.posicao.clone())
    let pontosCurva = curve.getPoints(numeroPontos)
    return pontosCurva.map(ponto => new No([ponto.x, ponto.y, ponto.z]))
  }
  desenharCurva() {
    if (this.nosSelecionados.length < 3) return
    const [a0, a1, a2] = this.nosSelecionados

    console.log("Desenhando curva para ", this.nosSelecionados)
    const curve = new CubicBezierCurve3(a0.posicao.clone(), a1.posicao.clone(), a1.posicao.clone(), a2.posicao.clone())
  ...
  desenharLinha(ponto1, ponto2, cena) {
    const material = new LineBasicMaterial({ color: 0xff00ff, linewidth: 2 })
