...
export default class GerenciadorNos {
    ...
    removeNo(no) {
        ...
        no.vizinhos
            ...
            .forEach((vizinho) => {
                const indexNo = vizinho.vizinhos
                    .map((noDistancia) => noDistancia.no)
                    .findIndex((noRef) => noRef == no)
                if (indexNo == -1) {
                    throw new Error("Esse nó não está mais na lista")
                    return
                }
    ...
    salvaNos() {
        const dadosArquivoJson = this.nos.map((no) => {
            const vizinhos = no.vizinhos.map((vizinho) => {
                ...
                const noIndex = this.nos.findIndex(
                    (noRef) => noRef == vizinho.no
                )
        ...
        fs.writeFileSync(
            path.resolve(__dirname, "nos.json"),
            JSON.stringify({ nos: dadosArquivoJson })
        )
        console.log("Arquivo nos.json salvo.")
    }
    carregarNos() {
        try {
            let dadosArquivoJson = JSON.parse(
                fs.readFileSync(path.resolve(__dirname, "nos.json"))
            )
            dadosArquivoJson = dadosArquivoJson.nos
            this.nos = dadosArquivoJson.map((no) => new No(no.posicao))
            this.nos.forEach((no, index) => {
                no.vizinhos = dadosArquivoJson[index].vizinhos.map(
                    (vizinho) => {
                        const { distanciaEntreNos } = vizinho
                        return {
